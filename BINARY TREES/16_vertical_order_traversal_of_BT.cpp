...
{
  map<int,map<int,multiset<int>>> hash_map;
  queue<pair<node* ,pair<int,int>>> qu;
  qu.push({root,{0,0}}); //initial vertical and level
  ...
  while (!qu.empty())
  {
    ...
    hash_map[x][y].insert(temp -> data); //inserting to multiset
...
// Output:The Vertical Traversal is : 
